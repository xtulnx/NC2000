...
void handle_cmd(string str){
	...
	if(cmds[0]=="put"){
			...
			if(nc1020mode){
				...
				Peek16(0x1231)=0xFF;  
				if(nc1020tw_mode){
					uint8_t buf[]={0x00,0x01,0x10,0xA9,0x00,0x8D,0xF6,0x03,0xAD,0xFF,0x3F,0xC9,0x00,0xF0,0x20,0xAD,
	0xFF,0x3F,0x8D,0x00,0x32,0xA9,0x00,0x8D,0x0D,0x12,0xA9,0x32,0x8D,0x0E,0x12,0xA9,
	0x01,0x8D,0x0F,0x12,0xA9,0x00,0x8D,0x10,0x12,0x00,0x05,0x10,0x4C,0x03,0x30,0x00,
	0x07,0x10,0x00,0x04,0x83,0x4C,0x32,0x30,};
					copy_to_addr(0x3000, buf, sizeof buf);
				}else{
